...
public class AnalyticsTest extends AndroidTest {
    ...
    public void analyticsPageFunctionality() {
        ...
        // Open analytics page
        driver.findElement(
            By.xpath("//android.view.View[@resource-id=\"btn_footer_analytics\"]")
        ).click();

        // Make sure the chart are filled in
        {
            boolean isElementExist = findElementOrNull(
                By.xpath("//android.view.View[@resource-id=\"sec_tasks_dist_empty_task\"]")
            );

            Assertions.assertEquals(
                false, 
                isElementExist, 
                "Tasks distribution chart is empty"
            );
        }
        ...
        {
            boolean isElementExist = findElementOrNull(
                By.xpath("//android.view.View[@resource-id=\"sec_top3_tasks_empty_task\"]")
            );

            Assertions.assertEquals(
                false, 
                isElementExist, 
                "List of top 3 urgent tasks is empty"
            );
        }
    }
}
